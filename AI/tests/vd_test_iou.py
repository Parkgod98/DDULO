...
class VideoIoUTestLogger:
    def __init__(self):
        self.all_results = []
        self.csv_filename = os.path.join("vdCSVs", f"vd_test_iou_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
        
    def log_result(self, iou_threshold, video_name, results):
        """테스트 결과 기록"""
        result_data = {
    ...
    def save_csv(self):
        ...
        # CSV 헤더
        headers = [
            'iou_threshold', 'video_name', 'timestamp', 'total_frames', 'avg_people_per_frame',
    ...
    def print_summary(self):
        ...
        # IoU별 통계
        print(f"{'IoU':<8} {'평균인원':<8} {'최대인원':<8} {'평균FPS':<8} {'처리시간(ms)':<12} {'테스트수':<8}")
        print("-" * 80)
        ...
        for iou in sorted(iou_groups.keys()):
            ...
            print(f"{iou:<8} {avg_people:<8.1f} {max_people:<8} {avg_fps:<8.1f} {avg_time:<12.1f} {len(results):<8}")
        ...
        if stable_results:
            ...
            print("   15FPS 이상의 IoU가 없습니다. 더 높은 IoU를 고려해보세요.")


def process_single_video_iou(video_path, model, roi_man, iou_threshold, conf_threshold=0.25, save_video=True):
    """단일 영상 처리 로직 (IoU 조정)"""
    ...
    if save_video:
        ...
        output_filename = f"{name}_{timestamp}_iou{iou_threshold}.mp4"
    ...
    while True:
        ...
        if save_video and video_writer:
            ...
            info_text1 = f"IoU:{iou_threshold} Frame:{frame_count+1}/{total_frames}"
...
def controlled_video_iou_test():
    """통제된 영상 IoU 테스트 메인 함수"""
    print("🎯 통제 영상 IoU 테스트 시작")
    print("=" * 50)
    ...
    for iou_threshold in iou_thresholds:
        ...
        for i, video_file in enumerate(video_files):
            ...
            # 비디오 처리
            result = process_single_video_iou(input_path, model, roi_man, iou_threshold, save_video=True)
            
            if result:
                # 결과 기록
                logger.log_result(iou_threshold, video_file, result)
